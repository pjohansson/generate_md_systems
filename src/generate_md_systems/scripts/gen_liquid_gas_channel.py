...
@dataclass
class Slab:
    """Thickness and position along z of a \"slab\" for a single phase."""
    thickness: float
    z0: float
...
def get_phase_slab_definitions(
    channel_width: float,
    ratio: float,
    separation: float,
    z0: float = 0.,
...
def parse_float_range(value: str, lower: float = 0., upper: float = 1.) -> float:
    result = float(value)

    if result < lower or result > upper:
        raise ValueError(f"must be in range [{lower}, {upper}]")
...
def main():
    ...
    parser.add_argument(
        'x',
        type=float,
        help="size of channel along x",
    )
    parser.add_argument(
        'y',
        type=float,
        help="size of channel along y",
    )
    parser.add_argument(
        'z',
        type=float,
        help="size of channel along z",
    )
    parser.add_argument(
        'ratio',
        type=parse_float_range,
        help="liquid/gas ratio in channel (0: all gas, 1: all liquid)",
    )

    parser_liquid = parser.add_argument_group('liquid phase options')
    parser_liquid.add_argument(
        '-l', '--liquid-conf',
    ...
    parser_liquid.add_argument(
        '--liquid-topolname',
        default='water-bulk', type=str, metavar='NAME',
        help="name of liquid molecule in topology file (default: %(default)s)",
    )
    parser_liquid.add_argument(
        '--liquid-residue-length',
        default=1, type=str, metavar='N',
        help="length of residue/molecule in liquid phase (default: %(default)s)",
    )

    parser_gas = parser.add_argument_group('gas phase options')
    parser_gas.add_argument(
        '-g', '--gas-conf',
        type=str, default=None, metavar='PATH',
        help="optional path to gas slab base configuration",
    )
    parser_gas.add_argument(
        '--gas-topolname',
        default='hydrogen-gas', type=str, metavar='NAME',
        help="name of gas molecule in topology file (default: %(default)s)",
    )
    parser_gas.add_argument(
        '--gas-residue-length',
        default=1, type=str, metavar='N',
        help="length of residue/molecule in gas phase (default: %(default)s)",
    )

    parser_fcc = parser.add_argument_group('substrate options')
    parser_fcc.add_argument(
        '-s', '--fcc-spacing',
