...
def get_slab_definitions(thicknesses, separations, z0=0.):
    if len(thicknesses) > 1 and len(separations) == 1:
        separations = separations * len(thicknesses)
    
    elif len(separations) < len(thicknesses) - 1:
        raise ValueError(f"number of `--separation` values must be 1 or equal to number of z values - 1 (was {len(separations)})")
...
def get_slabs_total_height(slabs):
    if slabs == []:
        return 0.
    else:
...
def adjust_slabs_z0(slabs, zadd):
    for slab in slabs:
        slab.z0 += zadd 
...
def calc_z_shifts(slabs, fcc_thickness, fcc_margin, box_margin):
